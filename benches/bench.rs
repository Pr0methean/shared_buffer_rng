...
fn benchmark_contended(c: &mut Criterion) {
    ...
    let background_threads: Vec<_> = (0..(num_cpus::get() - 1))
        .map(|_| {
            ...
            spawn(move || while !FINISHED.load(SeqCst) {
    ...
    background_threads.into_iter().for_each(|handle| handle.join().unwrap());
    FINISHED.store(false, SeqCst);
    let background_threads: Vec<_> = (0..(num_cpus::get() - 1))
        .map(|_| {
            let mut rng = ReseedingRng::new(ChaCha12Core::from_rng(OsRng::default()).unwrap(),
                                            RESEEDING_THRESHOLD, OsRng::default());
            spawn(move || while !FINISHED.load(SeqCst) {
                black_box(rng.next_u64());
            })
